...
export VISUAL=nano
# termux-zh
export PATH="$HOME/.termux-zh/bin:$PATH"

# 如果有 functions.sh，以后再补；现在不需要
#if [ -f "$HOME/.termux-zh/functions.sh" ]; then
#    source "$HOME/.termux-zh/functions.sh"
#fi

alias 恢复='bash ~/restore-all.sh'
alias 备份='bash ~/backup-all.sh'
alias 备份='bash ~/backup-all.sh'
alias 备份='bash ~/backup-all.sh'
alias 帮助='bash ~/help-all.sh'
